    ...
    for res_type in result_types:
        ...
        if single_result:
            ...
            for id_i in tgt_ids:
                ...
                cocoEval.accumulate()
                cocoEval.summarize()
                res_dict.update({coco.imgs[id_i]['file_name']:
                                 cocoEval.stats[1]})
...
def get_seg_masks(mask_pred, det_bboxes, det_labels,
                  img_meta, rescale, num_classes):
    ...
    for i in range(bboxes.shape[0]):
        ...
        if w <= 0 or h <= 0:
            print("there is invalid proposal bbox, \
                index={} bbox={} w={} h={}".format(i, bbox, w, h))
            w = max(w, 1)
            h = max(h, 1)
