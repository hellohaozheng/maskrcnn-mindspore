...
from .mobilenetv1 import MobileNetV1FeatureSelector
...
class MaskRcnnMobilenetv1(nn.Cell):
    ...
    def __init__(self, config):
        ...
        # Backbone mobilenetv1
        self.backbone = \
            MobileNetV1FeatureSelector(1001, features_only=True).to_float(self.platform_mstype)
        # Fpn
        self.fpn_neck = \
            FeatPyramidNeck(config.fpn_in_channels, config.fpn_out_channels, config.fpn_num_outs)

        # Rpn and rpn loss
        self.gt_labels_stage1 = \
            Tensor(np.ones((self.train_batch_size, config.num_gts)).astype(self.int_dtype))
        ...
        self.test_score_zeros = \
            Tensor(np.ones((self.rpn_max_num, 1)).astype(self.platform_dtype) * 0)
        self.test_box_zeros = \
            Tensor(np.ones((self.rpn_max_num, 4)).astype(self.platform_dtype) * -1)
        self.test_iou_thr = \
            Tensor(np.ones((self.rpn_max_num, 1)).astype(self.platform_dtype) * config.test_iou_thr)
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids, gt_masks):
        ...
        rpn_loss, cls_score, bbox_pred, rpn_cls_loss, rpn_reg_loss, _ = \
            self.rpn_with_loss(x, img_metas, self.anchor_list,
                               gt_bboxes, self.gt_labels_stage1, gt_valids)

        if self.training:
            proposal, proposal_mask = self.proposal_generator(cls_score, bbox_pred,
                                                              self.anchor_list)
        else:
        ...
        if self.training:
            for i in range(self.train_batch_size):
                ...
                bboxes, deltas, labels, mask, pos_bboxes, pos_mask_fb, pos_labels, pos_mask = \
                    self.bbox_assigner_sampler_for_rcnn(gt_bboxes_i, gt_labels_i,
                                                        proposal_mask[i],
                                                        proposal[i][::, 0:4:1],
                                                        gt_valids_i, gt_masks_i)
                bboxes_tuple += (bboxes,)
                deltas_tuple += (deltas,)
                labels_tuple += (labels,)
                mask_tuple += (mask,)
        ...
        if self.training:
            roi_feats = \
                self.roi_align(rois, self.cast(x[0], mstype.float32),
                               self.cast(x[1], mstype.float32),
                               self.cast(x[2], mstype.float32),
                               self.cast(x[3], mstype.float32))
        else:
            roi_feats = \
                self.roi_align_test(rois, self.cast(x[0], mstype.float32),
                                    self.cast(x[1], mstype.float32),
                                    self.cast(x[2], mstype.float32),
                                    self.cast(x[3], mstype.float32))
    ...
    def init_datatype(self):
        ...
        self.int_dtype = np.int32#uint8
        self.int_mstype = mstype.int32#uint8
    ...
    def init_tensors(self, config):
        ...
        self.rcnn_loss_cls_weight = \
            Tensor(np.array(config.rcnn_loss_cls_weight).astype(self.platform_dtype))
        self.rcnn_loss_reg_weight = \
            Tensor(np.array(config.rcnn_loss_reg_weight).astype(self.platform_dtype))
        self.rcnn_loss_mask_fb_weight = \
            Tensor(np.array(config.rcnn_loss_mask_fb_weight).astype(self.platform_dtype))
        ...
        self.test_mask_fb_zeros = \
            Tensor(np.zeros((self.rpn_max_num, 28, 28)).astype(self.platform_dtype))
