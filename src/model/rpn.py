...
class RPN(nn.Cell):
    ...
    def __init__(self,
        ...
        self.rpn_convs_list = \
            nn.layer.CellList(self._make_rpn_layer(self.num_layers,
                                                   in_channels,
                                                   feat_channels,
                                                   num_anchors,
                                                   cls_out_channels))
        ...
        self.rpn_loss_reg_weight = \
            Tensor(np.array(
                cfg_rpn.rpn_loss_reg_weight).astype(self.np_cast_type))
        self.rpn_loss_cls_weight = \
            Tensor(np.array(
                cfg_rpn.rpn_loss_cls_weight).astype(self.np_cast_type))
        expected_total_size = cfg_rpn.num_expected_neg * self.batch_size
        self.num_expected_total = \
            Tensor(np.array(expected_total_size).astype(self.np_cast_type))
        self.num_bboxes = cfg_rpn.num_bboxes
    ...
    def _make_rpn_layer(self, num_layers, in_channels,
        ...
        weight_conv = \
            initializer('Normal', shape=shp_weight_conv, dtype=mstype.float32)
        ...
        weight_cls = \
            initializer('Normal', shape=shp_weight_cls, dtype=mstype.float32)
        ...
        weight_reg = \
            initializer('Normal', shape=shp_weight_reg, dtype=mstype.float32)
        bias_reg = initializer(0, shape=shp_bias_reg, dtype=mstype.float32)

        for i in range(num_layers):
            rpn_layer.append(RpnRegClsBlock(in_channels,
                                            feat_channels,
                                            num_anchors,
                                            cls_out_channels,
                                            weight_conv,
    ...
    def construct(self, inputs, img_metas, anchor_list,
                  gt_bboxes, gt_labels, gt_valids):
        ...
        if self.training:
            for i in range(self.batch_size):
                ...
                for j in range(self.num_layers):
                    res = \
                        self.cast(
                            self.CheckValid(anchor_list[j],
                                            self.squeeze(img_metas[i:i + 1:1,
                                                                   ::])),
                            mstype.int32)
            ...
            for i in range(self.num_layers):
                ...
                loss_cls_item = self.sum_loss(loss_cls_item, (0,)) /\
                    self.num_expected_total

                loss_reg = self.loss_bbox(reg_score_i, bbox_target_)
                bbox_weight_ = self.tile(self.reshape(bbox_weight_,
                                         (self.feature_anchor_shape[i], 1)),
                                         (1, 4))
                loss_reg = loss_reg * bbox_weight_
                loss_reg_item = self.sum_loss(loss_reg, (1,))
                loss_reg_item = self.sum_loss(loss_reg_item, (0,)) /\
                    self.num_expected_total

                loss_total = self.rpn_loss_cls_weight * \
                    loss_cls_item + self.rpn_loss_reg_weight * loss_reg_item
