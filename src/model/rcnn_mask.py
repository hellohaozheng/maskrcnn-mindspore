...
def _conv(in_channels, out_channels, kernel_size=1,
          stride=1, padding=0, pad_mode='pad', gain=1):
    """Conv2D wrapper."""
    ...
    weights = \
        Tensor(np.random.normal(loc=0.0, scale=std,
    ...
    return nn.Conv2d(in_channels, out_channels,
                     ...
                     pad_mode=pad_mode, weight_init=weights,
                     has_bias=True, bias_init=bias)


def _conv_transpose(in_channels, out_channels, kernel_size=1,
                    stride=1, padding=0, pad_mode='pad',
                    gain=1):
...
class FpnMask(nn.Cell):
    ...
    def __init__(self, input_channels, output_channels, num_classes):
        ...
        self.mask_deconv5 = \
            _conv_transpose(output_channels,
                            output_channels,
                            kernel_size=2,
                            gain=2 ** 0.5,
                            stride=2,
        ...
        self.mask_conv6 = _conv(output_channels, num_classes,
                                kernel_size=1, stride=1, gain=2,
                                pad_mode="valid").to_float(self.cast_type)
...
class RcnnMask(nn.Cell):
    ...
    def __init__(self,
        ...
        self.rcnn_loss_mask_fb_weight = \
            Tensor(np.array(
                cfg.rcnn_loss_mask_fb_weight).astype(self.np_cast_type))
        ...
        self.fpn_mask = FpnMask(self.in_channels,
                                self.rcnn_mask_out_channels,
                                self.num_classes)
        ...
        self.rmv_first_tensor = \
            Tensor(rmv_first.astype(self.np_cast_type))
    ...
    def construct(self, mask_featuremap, labels=None, mask=None, mask_fb_targets=None):
        ...
        if self.training:
            bbox_weights = \
                self.cast(self.logicaland(self.greater(labels, 0), mask),
                          mstype.int32) * labels
            mask_fb_targets = \
                self.tile(self.expandims(mask_fb_targets, 1),
                          (1, self.num_classes, 1, 1))

            loss_mask_fb = self.loss(x_mask_fb, bbox_weights,
                                     mask, mask_fb_targets)
            out = loss_mask_fb
        else:
            out = x_mask_fb
    ...
    def loss(self, masks_fb_pred, bbox_weights, weights, masks_fb_targets):
        ...
        bbox_weights = self.cast(self.onehot(bbox_weights, self.num_classes,
