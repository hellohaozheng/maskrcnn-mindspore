...
class BboxAssignSampleForRcnn(nn.Cell):
    ...
    def __init__(self, config, batch_size, num_bboxes, add_gt_as_proposals):
        ...
        self.int_cast_type = np.int32#uint8
        ...
        self.label_inds = \
            Tensor(np.arange(1, self.num_gts + 1).astype(np.int32))
        self.add_gt_as_proposals_valid = \
            Tensor(np.array(self.add_gt_as_proposals * np.ones(self.num_gts),
                            dtype=np.int32))
        ...
        self.random_choice_with_mask_pos = \
            P.RandomChoiceWithMask(self.num_expected_pos)
        self.random_choice_with_mask_neg = \
            P.RandomChoiceWithMask(self.num_expected_neg)
        self.reshape = P.Reshape()
        self.equal = P.Equal()
        self.bounding_box_encode = \
            P.BoundingBoxEncode(means=(0.0, 0.0, 0.0, 0.0),
        ...
        # Check
        self.check_gt_one = \
            Tensor(np.array(-1 * np.ones((self.num_gts, 4)),
                            dtype=self.np_cast_type))
        self.check_anchor_two = \
            Tensor(np.array(-2 * np.ones((self.num_bboxes, 4)),
                            dtype=self.np_cast_type))

        # Init tensor
        self.assigned_gt_inds = \
            Tensor(np.array(-1 * np.ones(num_bboxes), dtype=np.int32))
        self.assigned_gt_zeros = \
            Tensor(np.array(np.zeros(num_bboxes), dtype=np.int32))
        self.assigned_gt_ones = \
            Tensor(np.array(np.ones(num_bboxes), dtype=np.int32))
        self.assigned_gt_ignores = \
            Tensor(np.array(-1 * np.ones(num_bboxes), dtype=np.int32))
        self.assigned_pos_ones = \
            Tensor(np.array(np.ones(self.num_expected_pos), dtype=np.int32))

        self.gt_ignores = \
            Tensor(np.array(-1 * np.ones(self.num_gts), dtype=np.int32))
        self.range_pos_size = \
            Tensor(np.arange(self.num_expected_pos).astype(self.np_cast_type))
        self.check_neg_mask = \
            Tensor(np.array(np.ones(self.num_expected_neg -
                                    self.num_expected_pos),
                            dtype=np.bool))
        self.bboxs_neg_mask = \
            Tensor(np.zeros((self.num_expected_neg, 4),
                            dtype=self.np_cast_type))
        self.labels_neg_mask = \
            Tensor(np.array(np.zeros(self.num_expected_neg), dtype=self.int_cast_type))
        ...
        self.scalar_neg_iou_thr = \
            Tensor(self.neg_iou_thr, dtype=self.cast_type)
        self.scalar_pos_iou_thr = \
            Tensor(self.pos_iou_thr, dtype=self.cast_type)
        self.scalar_min_pos_iou = \
            Tensor(self.min_pos_iou, dtype=self.cast_type)

        self.expand_dims = P.ExpandDims()
        self.split = P.Split(axis=1, output_num=4)
        self.concat_last_axis = P.Concat(axis=-1)
        self.round = P.Round()
        self.image_h_w = Tensor([cfg.img_height, cfg.img_width,
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask,
                  bboxes, gt_valids, gt_masks_i):
        """Construct Bbox assigner and sampler."""
        gt_bboxes_i = self.select(self.cast(self.tile(self.reshape(
                                  self.cast(gt_valids, mstype.int32),
                                  (self.num_gts, 1)), (1, 4)), mstype.bool_),
                                  gt_bboxes_i, self.check_gt_one)
        bboxes = self.select(self.cast(self.tile(self.reshape(
                             self.cast(valid_mask, mstype.int32),
        ...
        neg_sample_iou_mask = \
            self.logicaland(self.greaterequal(max_overlaps_w_gt,
                                              self.scalar_zero),
                            self.less(max_overlaps_w_gt,
                                      self.scalar_neg_iou_thr))

        assigned_gt_inds2 = self.select(neg_sample_iou_mask,
                                        self.assigned_gt_zeros,
                                        self.assigned_gt_inds)

        pos_sample_iou_mask = self.greaterequal(max_overlaps_w_gt,
                                                self.scalar_pos_iou_thr)
        assigned_gt_inds3 = \
            self.select(pos_sample_iou_mask,
        ...
        for j in range(self.num_gts):
            ...
            temp1 = \
                self.greaterequal(max_overlaps_w_ac_j, self.scalar_min_pos_iou)
            temp2 = \
                self.squeeze(self.equal(overlaps_w_ac_j, max_overlaps_w_ac_j))
            pos_mask_j = self.logicaland(temp1, temp2)
            assigned_gt_inds3 = \
                self.select(pos_mask_j, (j+1)*self.assigned_gt_ones,
                            assigned_gt_inds3)
        ...
        label_inds_valid = \
            self.select(gt_valids, self.label_inds, self.gt_ignores)
        ...
        pos_index, valid_pos_index = \
            self.random_choice_with_mask_pos(self.greater(
                assigned_gt_inds5, 0))

        pos_check_valid = \
            self.cast(self.greater(assigned_gt_inds5, 0), self.cast_type)
        pos_check_valid = self.sum_inds(pos_check_valid, -1)
        valid_pos_index = self.less(self.range_pos_size, pos_check_valid)

        pos_index = pos_index * \
            self.reshape(self.cast(valid_pos_index, mstype.int32),
        ...
        pos_assigned_gt_index = \
            self.gather_nd(assigned_gt_inds5,
                          pos_index) - self.assigned_pos_ones
        pos_assigned_gt_index = \
            self.reshape(pos_assigned_gt_index, self.reshape_shape_pos)
        ...
        # Get neg index
        neg_index, valid_neg_index = \
            self.random_choice_with_mask_neg(self.equal(assigned_gt_inds5, 0))

        unvalid_pos_index = self.less(self.range_pos_size, num_pos)
        valid_neg_index = self.logicaland(self.concat((self.check_neg_mask,
                                                       unvalid_pos_index)),
                                          valid_neg_index)
        ...
        pos_bbox_targets_ = \
            self.bounding_box_encode(pos_bboxes_, pos_gt_bboxes_)
        ...
        pos_masks_fb = self.crop_and_resize(pos_masks_fb, boxes,
        ...
        return(total_bboxes, total_deltas, total_labels,
               total_mask, pos_bboxes_, pos_masks_fb,
