...
class SingleRoIExtractor(nn.Cell):
    ...
    def set_train_local(self, config, training=True):
        ...
        self.batch_size = \
            roi_sample_num if self.training_local else cfg.rpn_max_num
        self.batch_size = self.train_batch_size*self.batch_size \
            if self.training_local else cfg.test_batch_size*self.batch_size
        self.ones = \
            Tensor(np.array(np.ones((self.batch_size, 1)), dtype=self.dtype))
        finest_scale = \
            np.array(np.ones((self.batch_size, 1)),
                     dtype=self.dtype) * self.finest_scale_
        self.finest_scale = Tensor(finest_scale)
        self.epslion = \
            Tensor(np.array(np.ones((self.batch_size, 1)),
                            dtype=self.dtype)*self.dtype(1e-6))
        self.zeros = \
            Tensor(np.array(np.zeros((self.batch_size, 1)),
                            dtype=np.int32))
        self.max_levels = \
            Tensor(np.array(np.ones((self.batch_size, 1)),
                            dtype=np.int32)*(self.num_levels-1))
        self.twos = \
            Tensor(np.array(np.ones((self.batch_size, 1)),
                            dtype=self.dtype) * 2)
        self.res_ = \
            Tensor(np.array(np.zeros((self.batch_size, self.out_channels,
