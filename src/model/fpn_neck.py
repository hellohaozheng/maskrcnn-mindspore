...
def bias_init_zeros(shape):
    """Bias init method."""
    result = \
        Tensor(np.array(np.zeros(shape).astype(np.float32)),
               dtype=mstype.float32)
    return result


def _conv(in_channels, out_channels, kernel_size=3,
...
class FeatPyramidNeck(nn.Cell):
    ...
    def __init__(self,
        ...
        for _, channel in enumerate(in_channels):
            l_conv = _conv(channel, out_channels,
                           kernel_size=1, stride=1, padding=0,
                           pad_mode='valid').to_float(self.cast_type)
            fpn_conv = _conv(out_channels, out_channels,
    ...
    def construct(self, inputs):
        """construction of Feat Pyramid Neck."""
        x = ()
        for i in range(self.fpn_layer):
            x += (self.lateral_convs_list[i](inputs[i]),)

        y = (x[3],)
        y = y + (x[2] + self.cast(self.interpolate1(y[self.fpn_layer - 4]),
                                  self.cast_type),)
        y = y + (x[1] + self.cast(self.interpolate2(y[self.fpn_layer - 3]),
                                  self.cast_type),)
        y = y + (x[0] + self.cast(self.interpolate3(y[self.fpn_layer - 2]),
                                  self.cast_type),)
