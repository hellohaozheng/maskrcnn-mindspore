...
class MaskRcnnResnet50(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.backbone = ResNetFea(ResidualBlockUsing,
                                  ...
                                  False)
        # self.backbone = SSDResNet50(ResidualBlock)
        # Fpn
        ...
        self.gt_labels_stage1 = \
            Tensor(np.ones((self.train_batch_size, config.num_gts)).astype(np.int32))
        ...
        self.bbox_assigner_sampler_for_rcnn = \
            BboxAssignSampleForRcnn(config, self.train_batch_size,
                                    config.num_bboxes_stage2, True)
        self.decode = \
            P.BoundingBoxDecode(max_shape=(768, 1280), means=self.target_means,
        ...
        # Rcnn
        self.rcnn_cls = \
            RcnnCls(config, self.train_batch_size, self.num_classes)
        self.rcnn_mask = \
            RcnnMask(config, self.train_batch_size, self.num_classes)
    ...
    def init_test_mode(self, config):
        ...
        self.test_iou_thr = \
            Tensor(np.ones((self.rpn_max_num, 1)).astype(self.np_cast_type) * config.test_iou_thr)
    ...
    def init_tensor(self, config):
        ...
        self.rcnn_loss_cls_weight = \
            Tensor(np.array(config.rcnn_loss_cls_weight).astype(self.np_cast_type))
        self.rcnn_loss_reg_weight = \
            Tensor(np.array(config.rcnn_loss_reg_weight).astype(self.np_cast_type))
        self.rcnn_loss_mask_fb_weight = \
            Tensor(np.array(config.rcnn_loss_mask_fb_weight).astype(self.np_cast_type))
        ...
        self.test_mask_fb_zeros = \
            Tensor(np.zeros((self.rpn_max_num, 28, 28)).astype(self.np_cast_type))
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids, gt_masks):
        ...
        rpn_loss, cls_score, bbox_pred, \
            rpn_cls_loss, rpn_reg_loss, _ = self.rpn_with_loss(x, img_metas,
                                                               self.anchor_list,
                                                               gt_bboxes,
                                                               self.gt_labels_stage1,
                                                               gt_valids)
        ...
        if self.training:
            proposal, proposal_mask = \
                self.proposal_generator(cls_score, bbox_pred, self.anchor_list)
        else:
            proposal, proposal_mask = \
                self.proposal_generator_test(cls_score, bbox_pred, self.anchor_list)
        ...
        if self.training:
            return self.get_output_train(pos_rois, x, rcnn_pos_labels,
                                         rcnn_pos_mask_squeeze, rcnn_pos_masks_fb,
                                         rpn_loss, rpn_cls_loss, rpn_reg_loss,
                                         rcnn_cls_loss, rcnn_reg_loss)

        return self.get_output_eval(x, bboxes_all, rcnn_cls_loss,
                                    rcnn_reg_loss, rcnn_masks, img_metas)
    ...
    def get_output_train(self, pos_rois, x, rcnn_pos_labels,
                         rcnn_pos_mask_squeeze, rcnn_pos_masks_fb,
                         rpn_loss, rpn_cls_loss, rpn_reg_loss,
                         rcnn_cls_loss, rcnn_reg_loss):
    ...
    def get_output_eval(self, x, bboxes_all, rcnn_cls_loss,
                        rcnn_reg_loss, rcnn_masks, img_metas):
    ...
    def get_det_bboxes(self, cls_logits, reg_logits, mask_logits,
                       rois, img_metas, mask_fb_pred_all):
    ...
    def multiclass_nms(self, boxes_all, scores_all, mask_all, mask_fb_all):
        ...
        for i in range(self.test_batch_size):
            ...
            for j in range(self.num_classes - 1):
                ...
                _reg_mask = self.cast(self.tile(self.cast(_mask, mstype.int32),
                                                (1, 4)), mstype.bool_)

                _bboxes = self.select(_reg_mask, _bboxes, self.test_box_zeros)
                _fb_mask = self.expand_dims(_mask, -1)
                _mask_fb_mask = self.cast(self.tile(self.cast(_fb_mask, mstype.int32),
                                                    (1, 28, 28)), mstype.bool_)
...
class MaskRcnnInfer(nn.Cell):
    ...
    """
    def __init__(self, config):
        super(MaskRcnnInfer, self).__init__()
        self.network = MaskRcnnResnet50(config)
