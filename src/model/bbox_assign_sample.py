...
class BboxAssignSample(nn.Cell):
    ...
    Returns:
        ...
        label_weights: label weight for every bboxes,
    ...
    def __init__(self, config, batch_size,
                 num_bboxes, add_gt_as_proposals):
        ...
        self.assigned_gt_inds = \
            Tensor(np.array(-1 * np.ones(num_bboxes),
                            dtype=np.int32))
        self.assigned_gt_zeros = \
            Tensor(np.array(np.zeros(num_bboxes),
                            dtype=np.int32))
        self.assigned_gt_ones = \
            Tensor(np.array(np.ones(num_bboxes),
                            dtype=np.int32))
        self.assigned_gt_ignores = \
            Tensor(np.array(-1 * np.ones(num_bboxes),
                            dtype=np.int32))
        self.assigned_pos_ones = \
            Tensor(np.array(np.ones(self.num_expected_pos),
                            dtype=np.int32))
        ...
        self.check_anchor_two = \
            Tensor(np.array(-2 * np.ones((self.num_bboxes, 4)),
                            dtype=np.float32))
        self.range_pos_size = \
            Tensor(np.arange(self.num_expected_pos).astype(np.float32))


    def construct(self, gt_bboxes_i, gt_labels_i,
                  valid_mask, bboxes, gt_valids):
        """construction of bbox assign sample."""
        gt_valids_reshape = \
            self.reshape(self.cast(gt_valids, mstype.int32), (self.num_gts, 1))
        gt_valids_target = \
            self.cast(self.tile(gt_valids_reshape, (1, 4)), mstype.bool_)
        gt_bboxes_i = \
            self.select(gt_valids_target, gt_bboxes_i, self.check_gt_one)

        bboxes_valid_reshape = \
            self.reshape(self.cast(valid_mask, mstype.int32),
                         (self.num_bboxes, 1))
        bboxes_valid_target = \
            self.cast(self.tile(bboxes_valid_reshape, (1, 4)), mstype.bool_)
        bboxes = \
            self.select(bboxes_valid_target, bboxes, self.check_anchor_two)
        ...
        pos_sample_iou_mask = \
            self.greaterequal(max_overlaps_w_gt, self.pos_iou_thr)
        max_overlaps_w_gt_inds = \
        ...
        for j in range(self.num_gts):
            ...
            pos_mask_j = self.logicaland(self.greaterequal(max_overlaps_w_ac_j,
                                                           self.min_pos_iou),
                                         self.equal(overlaps_w_gt_j,
                                                    max_overlaps_w_ac_j))

            assigned_gt_inds4 = self.select(pos_mask_j,
                                            self.assigned_gt_ones + j,
                                            assigned_gt_inds4)

        assigned_gt_inds5 = self.select(valid_mask,
                                        assigned_gt_inds4,
                                        self.assigned_gt_ignores)
        ...
        pos_check_valid = \
            self.cast(self.greater(assigned_gt_inds5, 0),
                      self.cast_type)
        pos_check_valid = self.sum_inds(pos_check_valid, -1)
        valid_pos_index = self.less(self.range_pos_size, pos_check_valid)
        pos_index = \
            pos_index * self.reshape(self.cast(valid_pos_index, mstype.int32),
                                     (self.num_expected_pos, 1))

        pos_assigned_gt_index = \
            self.gatherND(assigned_gt_inds5, pos_index) - self.assigned_pos_ones
        pos_assigned_gt_index = \
            pos_assigned_gt_index * self.cast(valid_pos_index, mstype.int32)
        pos_assigned_gt_index = \
            self.reshape(pos_assigned_gt_index, (self.num_expected_pos, 1))
        ...
        valid_neg_index = \
            self.logicaland(self.concat((self.check_neg_mask,
                                         unvalid_pos_index)),
                            valid_neg_index)
        ...
        bbox_targets_total = \
            self.scatterNd(pos_index, pos_bbox_targets_, (self.num_bboxes, 4))
        bbox_weights_total = \
            self.scatterNd(pos_index, valid_pos_index, (self.num_bboxes,))
        labels_total = \
            self.scatterNd(pos_index, pos_gt_labels, (self.num_bboxes,))
        total_index = self.concat((pos_index, neg_index))
        total_valid_index = self.concat((valid_pos_index, valid_neg_index))
        label_weights_total = \
            self.scatterNd(total_index, total_valid_index, (self.num_bboxes,))

        return(bbox_targets_total,
