...
class Proposal(nn.Cell):
    ...
    def __init__(self,
        ...
        self.decode = \
            P.BoundingBoxDecode(max_shape=(cfg.img_height,
                                           cfg.img_width),
    ...
    def set_train_local(self, config, training=True):
        ...
        self.topk_mask = \
            Tensor(self.min_float_num * np.ones(total_max_topk_input,
                                                self.np_cast_type))
    ...
    def construct(self, rpn_cls_score_total, rpn_bbox_pred_total, anchor_list):
        ...
        for img_id in range(self.batch_size):
            ...
            proposals, masks = self.get_bboxes_single(cls_score_list,
                                                      bbox_pred_list,
                                                      anchor_list)
    ...
    def get_bboxes_single(self, cls_scores, bbox_preds, mlvl_anchors):
        ...
        for idx in range(self.num_levels):
            rpn_cls_score = \
                self.transpose(cls_scores[idx], self.transpose_shape)
            rpn_bbox_pred = \
                self.transpose(bbox_preds[idx], self.transpose_shape)
            anchors = mlvl_anchors[idx]

            rpn_cls_score = self.reshape(rpn_cls_score, self.reshape_shape)
            rpn_cls_score = self.activation(rpn_cls_score)
            rpn_cls_score_process = \
                self.cast(self.squeeze(rpn_cls_score[::, 0::]), self.cast_type)

            rpn_bbox_pred_process = \
                self.cast(self.reshape(rpn_bbox_pred, (-1, 4)), self.cast_type)

            scores_sorted, topk_inds = \
                self.topk_v2(rpn_cls_score_process, self.topk_stage1[idx])

            topk_inds = self.reshape(topk_inds, self.topk_shape[idx])

            bboxes_sorted = self.gatherND(rpn_bbox_pred_process, topk_inds)
            anchors_sorted = \
                self.cast(self.gatherND(anchors, topk_inds), self.cast_type)

            proposals_decode = self.decode(anchors_sorted, bboxes_sorted)

            proposals_decode = \
                self.concat_axis1((proposals_decode,
                                   self.reshape(scores_sorted,
                                                self.topk_shape[idx])))
