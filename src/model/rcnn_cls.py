...
class DenseNoTranpose(nn.Cell):
    ...
    def __init__(self, input_channels, output_channels, weight_init):
        ...
        self.weight = \
            Parameter(initializer(weight_init,
                                  [input_channels,
                                   output_channels],
                                  mstype.float32))
        self.bias = \
            Parameter(initializer("zeros",
                                  [output_channels],
                                  mstype.float32))
...
class FpnCls(nn.Cell):
    ...
    def __init__(self, input_channels,
                 output_channels, num_classes, pool_size):
        ...
        weights_0 = \
            initializer("XavierUniform", shape=shape_0[::-1],
                        dtype=mstype.float32)
        shape_1 = (output_channels, output_channels)
        weights_1 = \
            initializer("XavierUniform", shape=shape_1[::-1],
                        dtype=mstype.float32)
        self.shared_fc_0 = \
            DenseNoTranpose(representation_size, output_channels,
                            weights_0).to_float(self.cast_type)
        self.shared_fc_1 = \
            DenseNoTranpose(output_channels, output_channels,
                            weights_1).to_float(self.cast_type)

        cls_weight = \
            initializer('Normal', shape=[num_classes,
                        output_channels][::-1],
                        dtype=mstype.float32)
        reg_weight = \
            initializer('Normal', shape=[num_classes * 4,
                        output_channels][::-1],
                        dtype=mstype.float32)
        self.cls_scores = \
            DenseNoTranpose(output_channels, num_classes,
                            cls_weight).to_float(self.cast_type)
        self.reg_scores = \
            DenseNoTranpose(output_channels, num_classes * 4,
                            reg_weight).to_float(self.cast_type)
...
class RcnnCls(nn.Cell):
    ...
    Args:
        ...
        target_means (list) - Means for encode function.
                            Default: (.0, .0, .0, .0]).
        target_stds (list) - Stds for encode function.
    ...
    def __init__(self,
        ...
        self.rcnn_loss_cls_weight = \
            Tensor(np.array(
                cfg.rcnn_loss_cls_weight).astype(self.np_cast_type))
        self.rcnn_loss_reg_weight = \
            Tensor(np.array(
                cfg.rcnn_loss_reg_weight).astype(self.np_cast_type))
        ...
        self.fpn_cls = \
            FpnCls(self.in_channels, self.rcnn_fc_out_channels,
    ...
    def construct(self, featuremap, bbox_targets, labels, mask):
        ...
        if self.training:
            bbox_weights = \
                self.cast(self.logicaland(self.greater(labels, 0),
                                          mask), mstype.int32) * labels
            labels = \
                self.cast(self.onehot(labels, self.num_classes,
                                      self.on_value,
                                      self.off_value), self.cast_type)
            bbox_targets = \
                self.tile(self.expandims(bbox_targets, 1),
    ...
    def loss(self, cls_score, bbox_pred, bbox_targets,
             bbox_weights, labels, weights):
        """Loss method."""
        # loss_cls
        loss_cls, _ = self.loss_cls(cls_score, labels)
        weights = self.cast(weights, self.cast_type)
        loss_cls = loss_cls * weights
        loss_cls = self.sum_loss(loss_cls, (0,)) /\
            (self.sum_loss(weights, (0,)) + 1e-5)
